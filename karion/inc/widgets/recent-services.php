<?php
add_action('widgets_init', 'cs_recent_services_carousel');

function cs_recent_services_carousel() {
    register_widget('CS_Recent_services_Carousel');
}

class CS_Recent_services_Carousel extends WP_Widget {

    function __construct() {
        parent::__construct(
                'cs_recent_services_carousel', esc_html__('CMS Recent Services', 'construct'), array('description' => esc_html__('Recent Services Widget.', 'construct'),)
        );
    }
    ...
    function widget($args, $instance) {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Recent services', 'construct' ) : $instance['title'], $instance, $this->id_base);
        ...
        echo '<aside class="widget widget_nav_menu">';
            ...
            <?php if ($wp_query->have_posts()){ ?>
                <div class="cms-recent-post">
                    <ul class="cms-recent-services-wrapper menu">
    ...
    function form($instance) {
        ...
        ?>
        <p>
            <label for="<?php echo ''.$this->get_field_id('title'); ?>"><?php esc_html_e( 'Title:', 'construct' ); ?></label>
        ...
        <p>
            <label for="<?php echo ''.$this->get_field_id('number'); ?>"><?php esc_html_e( 'Number of post to show:', 'construct' ); ?></label>
