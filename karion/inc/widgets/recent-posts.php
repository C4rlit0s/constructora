...
class CS_Recent_Post_Carousel extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'cs_recent_post_carousel', esc_html__('CMS Recent Post', 'construct'), array('description' => esc_html__('Recent Posts Widget.', 'construct'),)
        );
    }
    ...
    function widget($args, $instance) {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Recent Posts', 'construct' ) : $instance['title'], $instance, $this->id_base);
        $show_date = (int) $instance['show_date'];
        $show_decs = (int) $instance['show_decs'];
        $number = (int) $instance['number'];

        echo '<aside id="cms-recent-post" class="widget">';
            if($title) {
                    ...
                    <ul class="cms-recent-post-wrapper wg-menu-item">
                        <?php global $post; while ($wp_query->have_posts()): $wp_query->the_post(); ?>
                            <li class="item clearfix">
                                ...
                                <span class="date-post-time"><?php construct_post_detail(); ?></span>
                            </li>
                        <?php endwhile; ?>
                    </ul>
                </div>
